...
```sh
npm install --save-dev @embody/testing
```

Requires Node.js 22 or 24 and uses ESM. `createTestHarness` boots the public `Kernel`, SQLite adapter, execution path, and deterministic outbox worker. Always call `await harness.close()` (normally in `afterEach`) to remove its isolated temporary database.
